...
if (Meteor.isClient) {
    ...
    maliciousInputs.forEach((input, index) => {
      setTimeout(() => {
        Accounts.createUser({
          ...
          if (error) {
            console.log(`Malicious input test ${index + 1}: BLOCKED - ${error.reason}`);
          } else {
            console.log(`Malicious input test ${index + 1}: ALLOWED (Security issue!)`);
          }
        });
      }, index * 2000);
    });
  ...
  testSecurityHeaders = function() {
    ...
    fetch('/', {
      ...
      const essentialHeaders = ['X-Content-Type-Options', 'X-Frame-Options', 'X-XSS-Protection'];
  ...
  Meteor.startup(() => {
    ...
    console.log('- testAccountCreationRateLimit() - Test account creation limits');
